...
function initWireframeGlobe() {
  const canvas = document.getElementById('exportGlobeCanvas');
  if (!canvas) return;
  ...
  function draw() {
    ...
    for (let lat = -60; lat <= 60; lat += 20) {
      ...
      ctx.beginPath();
      ctx.ellipse(cx, y, Math.max(0.1, r), Math.max(0.1, r * 0.28), 0, 0, Math.PI * 2);
      ctx.stroke();
    }
    ...
    for (let lon = 0; lon < 180; lon += 24) {
      ...
      ctx.beginPath();
      ctx.ellipse(cx, cy, Math.max(0.1, rx), radius, 0, 0, Math.PI * 2);
      ctx.stroke();
    }
    ...
    // Glowing Export Hub Pins
    const hubPoints = [
      { lat: 37, lon: -95 }, // USA
      { lat: 55, lon: -3 },   // UK
      { lat: 48, lon: 2 },    // Europe
      { lat: -25, lon: 133 }, // Australia
      { lat: 26, lon: 80 },   // India HQ
      { lat: 35, lon: 139 },  // Japan
      { lat: 36, lon: 127 },  // South Korea
      { lat: -41, lon: 174 }, // NZ
      { lat: 23, lon: 53 }    // UAE
    ];
    ...
    hubPoints.forEach(pt => {
      ...
      const radLon = (pt.lon + rotation) * Math.PI / 180;
      ...
      if (z > 0) {
        ctx.fillStyle = pt.lon === 80 ? '#e11d48' : '#f97316';
        ctx.shadowColor = '#f97316';
        ctx.shadowBlur = 10;
        ctx.beginPath();
        ctx.arc(x, y, pt.lon === 80 ? 5 : 4, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;
      }
    });
